...
class DockerHostManager:
	...
	def _resolve_endpoint(self, context_name, hostname):
		context_file = os.path.expanduser(
			f'~/.docker/contexts/meta/{context_name}/meta.json'
		)
	...
	def run_container(self, context_name, image, name, env, ports, shm_size=None,
					  memory=None, nano_cpus=None):
		return self._bridge.run(
			self._run_container_async(context_name, image, name, env, ports,
									  shm_size, memory, nano_cpus)
		)

	async def _run_container_async(self, context_name, image, name, env, ports,
								   shm_size, memory, nano_cpus):
		client = self._clients.get(context_name)
		if not client:
			raise Exception(f"no client for context '{context_name}'")
	...
	def get_ports(self, context_name, container_id_or_name):
		return self._bridge.run(
			self._get_ports_async(context_name, container_id_or_name)
		)
	...
	def stop_container(self, context_name, container_name, timeout=10):
		try:
			self._bridge.run(
				self._stop_container_async(context_name, container_name, timeout)
			)
		except Exception as e:
