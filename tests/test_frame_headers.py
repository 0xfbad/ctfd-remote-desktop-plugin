...
def loaded_plugin():
    ...
    fake_open.return_value.__exit__ = lambda *_args, **_kwargs: None
...
def _lifecycle_schema_inspector(active_foreign_keys=()):
    inspector = MagicMock()
    columns = {
        ...
        "desktop_session_history": [{"name": "session_uuid"}],
        ...
        "desktop_event_log": [{"name": "event_id", "type": SimpleNamespace(length=128)}],
    }
    uniques = {
        "desktop_container_info": [
            {"column_names": ["user_id"]},
            {"column_names": ["session_uuid"]},
        ],
        "desktop_session_history": [{"column_names": ["session_uuid"]}],
        "desktop_session_operations": [
            {"column_names": ["operation_uuid"]},
            {"column_names": ["session_uuid"]},
        ],
        "desktop_event_log": [{"column_names": ["event_id"]}],
    }
    inspector.get_table_names.return_value = list(columns)
    inspector.get_columns.side_effect = lambda table: columns[table]
    inspector.get_unique_constraints.side_effect = lambda table: uniques[table]
    inspector.get_indexes.return_value = []
    inspector.get_foreign_keys.return_value = list(active_foreign_keys)
    return inspector


def test_schema_validation_rejects_cascading_active_user_fk(loaded_plugin):
    mod, app, _bp = loaded_plugin
    inspector = _lifecycle_schema_inspector(
        [{"constrained_columns": ["user_id"], "referred_table": "users", "name": "invalid_user_fk"}]
    )

    with patch("sqlalchemy.inspect", return_value=inspector, create=True):
        with pytest.raises(RuntimeError, match="user_id must not have a foreign key"):
            mod._validate_database_schema(app)
